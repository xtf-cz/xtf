	...
	public BuildConfig build() {
		...
		if (imageSource != null) {
			...
			imageSource.getPaths().forEach(
					x -> isb.addNewPath().withDestinationDir(x.getFirst())
							.withSourcePath(x.getSecond()).endPath());
		...
		// spec
		final SpecNested<io.fabric8.openshift.api.model.BuildConfigBuilder> spec = 
				builder.withNewSpec();
		...
		{
			io.fabric8.openshift.api.model.BuildConfigSpecFluent.ResourcesNested<SpecNested<io.fabric8.openshift.api.model.BuildConfigBuilder>> resources = spec.withNewResources();
			if (!requests.isEmpty()) {
