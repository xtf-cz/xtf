...
public class MySQL extends AbstractSQLDatabase {
	...
	protected void configureContainer(ContainerBuilder containerBuilder) {
		if (withLivenessProbe) containerBuilder.addLivenessProbe().setInitialDelay(30).createTcpProbe("3306");
		if (withReadinessProbe) containerBuilder.addReadinessProbe().setInitialDelaySeconds(5).createExecProbe("/bin/sh", "-i", "-c", "MYSQL_PWD=\"$MYSQL_PASSWORD\" mysql -h 127.0.0.1 -u $MYSQL_USER -D $MYSQL_DATABASE -e 'SELECT 1'");
	}
