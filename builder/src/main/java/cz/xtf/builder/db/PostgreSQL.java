...
public class PostgreSQL extends AbstractSQLDatabase {
	...
	protected void configureContainer(ContainerBuilder containerBuilder) {
		if (withLivenessProbe) containerBuilder.addLivenessProbe().setInitialDelay(300).createTcpProbe("5432");
		if (withReadinessProbe) containerBuilder.addReadinessProbe().setInitialDelaySeconds(5).createExecProbe("/bin/sh", "-i", "-c", "psql -h 127.0.0.1 -U $POSTGRESQL_USER -q -d $POSTGRESQL_DATABASE -c 'SELECT 1'");
	}
