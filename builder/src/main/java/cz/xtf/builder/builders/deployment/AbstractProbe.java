...
public abstract class AbstractProbe {
	...
	public Probe build() {
		...
		if (probeType.equals("httpGet")) {
			final List<HTTPHeader> headers = httpHeaders.entrySet().stream().map(
					x -> (new HTTPHeaderBuilder()).withName(x.getKey()).withValue(x.getValue()).build()).collect(Collectors.toList());
	...
	public AbstractProbe createHttpProbe(final String host, final String path, final String port, final Map<String, String> httpHeaders) {
