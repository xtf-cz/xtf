...
public class ContainerBuilder implements EnvironmentConfiguration, ResourceLimitBuilder {
	...
	public Container build() {
		...
		Stream<EnvVar> referredVars = referredEnvVars.entrySet().stream().map(entry -> new EnvVar(entry.getKey(), null, new EnvVarSource(new ConfigMapKeySelectorBuilder().withKey(entry.getValue().getKey()).withName(entry.getValue().getValue()).build(), null, null, null)));
